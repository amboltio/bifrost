...
func TestIdentityAuthorizationMiddlewareDeniesUnmappedCanonicalManagementRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "profile manager reads role defaults", method: fasthttp.MethodGet, path: "/api/governance/roles/engineering/access-profiles", roles: []tables.TableRole{{ID: "profiles", Permissions: []string{string(authorization.PermissionAccessProfilesRead)}}}, userID: "user", wantCalled: true, wantStatus: fasthttp.StatusOK},
		{name: "profile assigner updates role defaults", method: fasthttp.MethodPut, path: "/api/governance/roles/engineering/access-profiles", roles: []tables.TableRole{{ID: "profiles", Permissions: []string{string(authorization.PermissionAccessProfilesAssign)}}}, userID: "user", wantCalled: true, wantStatus: fasthttp.StatusOK},
		{name: "profile reader cannot update role defaults", method: fasthttp.MethodPut, path: "/api/governance/roles/engineering/access-profiles", roles: []tables.TableRole{{ID: "profiles", Permissions: []string{string(authorization.PermissionAccessProfilesRead)}}}, userID: "user", wantStatus: fasthttp.StatusForbidden},
